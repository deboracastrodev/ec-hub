...
| Métricas HTTP globais (Story 8.4) num único hash Redis `ec-hub:http-metrics` (`HINCRBY`/`HINCRBYFLOAT` em `MULTI/EXEC`, um `HGETALL` na leitura), gravadas pelo `public/index.php` ao fim de cada requisição roteada, com o rótulo de rota do `Router` (`/products/{param}`, `unmatched`). `GET /api/metrics?format=json\|prometheus` agrega esse hash com as fontes que já existiam (`MemoryMonitor`, `RecommendationExperiment::results()`, `EventBusStatusInterface`), cada uma num `try/catch` próprio | O PHP é share-nothing (`php -S`/FPM): um contador em memória de processo zera a cada requisição. Um hash com campos compostos evita `KEYS`/`SCAN`; o bucket gravado não é cumulativo (3 comandos por escrita) e a soma cumulativa é feita na leitura. O rótulo vem da rota, nunca da URI, para a cardinalidade não crescer com ids | Cliente Prometheus via Composer (dependência nova para um formato de texto simples); APCu ou arquivo local (não é compartilhado entre processos/containers de forma confiável); uma chave Redis por rota (exigiria `SCAN` na leitura); um segundo registro de métricas por algoritmo (o 8.2 já tem o seu) |
| Painel admin (Story 8.3) com um único admin: `ADMIN_USERNAME` + `ADMIN_PASSWORD_HASH` (`password_hash`) no ambiente, sessão num cookie `ec_hub_admin` assinado por HMAC (`SESSION_COOKIE_SECRET`, TTL 2 h, `SameSite=Strict`, `path=/admin`) e CSRF stateless (HMAC do cookie; no login, HMAC do id da sessão do visitante). Sem credenciais válidas o painel fica desligado (fail-closed). Exclusão de produto é soft delete (`products.deleted_at`), filtrado em todas as leituras do `ProductRepository` | Não há usuários no domínio e a sessão Redis não tem remoção de campo: o cookie stateless não depende do Redis e evita fixação de sessão (o token só nasce depois do login). O hash da senha entra na assinatura, então trocar a senha derruba as sessões abertas. Custo aceito: não dá para revogar uma sessão antes de expirar, e o login não tem rate limit (documentado em DEPLOYMENT.md §11) | Tabela de usuários com papéis/ACL (escopo maior que um CRUD de catálogo); HTTP Basic (sem logout nem CSRF); sessão admin no Redis (exigiria mudar o `SessionRepositoryInterface`); hard delete (quebraria FR106 e o histórico de eventos que cita o produto) |
| Checkout simulado (Story 8.7): o `PlaceOrder` primeiro "reivindica" o carrinho (`compareAndSwap` de `cart.items` para `[]` contra o valor lido, até 5 tentativas) e só depois grava o pedido no MySQL (`orders` + `order_items` numa transação, preços como snapshot em DECIMAL). Se o `INSERT` falhar, o carrinho é devolvido best effort; o email de confirmação é um arquivo `var/mail/{número}.eml` e sua falha não desfaz o pedido. A página de confirmação só abre o pedido gravado em `checkout.last_order` na sessão de quem comprou | O CAS antes do `INSERT` impede pedido duplicado no duplo clique (o segundo POST lê o carrinho já vazio). MySQL e Redis não têm transação comum: o pior caso é um carrinho perdido, nunca um pedido fantasma. O `.eml` deixa inspecionar o que "teria sido enviado" sem SMTP (o logger do container é `NullLogger`) | Gravar o pedido antes de limpar o carrinho (duplica pedido no duplo submit); SMTP ou serviço de email (dependência e segredo novos para um POC); confirmação por URL com o número do pedido (expõe dados pessoais a quem tiver o link) |
