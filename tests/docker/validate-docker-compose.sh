...
if ! grep -q "redis:" "$COMPOSE_FILE" || ! grep -q "redis:7-alpine" "$COMPOSE_FILE"; then
...
if ! grep -A 12 '^  redis:' "$COMPOSE_FILE" | grep -q 'redis-cli.*ping'; then
...
if ! grep -A 35 '^  app:' "$COMPOSE_FILE" | grep -q "mysql:"; then
...
if ! grep -A 35 '^  app:' "$COMPOSE_FILE" | grep -q "depends_on"; then
  echo "❌ FAIL: App service must depend on mysql"
  ERRORS=$((ERRORS + 1))
else
...
if ! grep -A 35 '^  app:' "$COMPOSE_FILE" | grep -q 'redis:'; then
