...
run_test "Redis and session configuration in .env.example" "grep -q '^REDIS_HOST=' .env.example && grep -q '^REDIS_PORT=' .env.example && grep -q '^SESSION_TTL=' .env.example && grep -q '^SESSION_COOKIE_SECRET=' .env.example"
echo ""
...
run_test "App receives Redis environment" \
  "docker exec ec-hub-app php -r \"exit(getenv('REDIS_HOST') === 'redis' && getenv('REDIS_PORT') === '6379' && getenv('SESSION_TTL') === '${SESSION_TTL:-1800}' && strlen((string) getenv('SESSION_COOKIE_SECRET')) >= 32 ? 0 : 1);\""
