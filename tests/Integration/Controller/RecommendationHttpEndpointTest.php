...
class RecommendationHttpEndpointTest extends TestCase
{
    ...
    #[RunInSeparateProcess]
    public function testApiEndpointMovesCurrentSnapshotToPrevious(): void
    {
        header_remove();
        http_response_code(200);
        ...
        $generateRecommendations->expects($this->once())
            ->method('execute')
            ->with(1, 10, false, null, $sessionId, null)
            ->willReturn([['product_id' => 22, 'score' => 75.0]]);
        $sessions = new HttpRecommendationSessionRepository(false, [
...
final class HttpRecommendationSessionRepository implements SessionRepositoryInterface
{
    ...
    public function __construct(private readonly bool $throwsOnSave = false, private array $data = [])
    {
    }
