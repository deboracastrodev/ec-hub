...
- **O CF relê o event store a cada requisição, com dois limites de custo (DW-13).** O CF não tem cache de modelo (o da Story 10.1 cobre só o KNN), então cada requisição com `collaborative` faz três `getByEvent()` e recalcula a co-ocorrência. Dois limites seguram esse custo:
  - **Retenção por lista de evento:** o `RedisEventStore` guarda só os N envelopes mais recentes de cada evento (`EVENT_STORE_MAX_EVENTS_PER_LIST`, padrão 5000; valor inválido gera `InvalidArgumentException` ao resolver o event store). A cada gravação, os envelopes mais antigos além de N são descartados de vez. Então, depois da próxima gravação de cada evento, cada requisição lê no máximo 3 × N envelopes (uma lista que já passava de N antes da mudança só é aparada quando recebe um novo evento).
  - **Teto de produtos por sessão:** no `train()`, cada sessão conta em no máximo 50 produtos distintos (padrão do parâmetro do construtor do `CollaborativeFilteringService`, sem variável de ambiente). Os primeiros 50 são os que aparecem primeiro na ordem de leitura (`product.viewed`, depois `product.clicked`, depois `cart.item_added`, cada lista na ordem gravada). Um produto além disso é ignorado para aquela sessão: ela não conta nem em |S_p| nem na co-ocorrência dele. A co-ocorrência de uma sessão fica em no máximo 50² pares, e uma sessão de bot é truncada, não descartada.

  O custo por requisição agora tem teto, mas continua **não medido**: não há número que mostre se o CF cabe no NFR de < 200 ms.
...
- **CF depende de histórico.** Sem interações registradas (base nova, Redis limpo) o CF não recomenda nada e toda resposta vem do fallback. A retenção descarta por volume (só os N envelopes mais recentes de cada evento), não por idade: dentro da janela retida, interesse antigo pesa igual a recente, e num tráfego baixo eventos antigos podem ficar retidos por muito tempo.
